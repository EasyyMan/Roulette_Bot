...
public class Rebet_Btn : UIButtonsBase
{
    private readonly string _clickEventName = "rebetPlaced";
    private readonly TargetBox _box;
    private readonly ICasinoSiteService _casinoSite;

    public Rebet_Btn(IWebDriver? driver, IPythonEnvironment? python, ObservableCollection<TargetBox> boxes, ICasinoSiteService casinoSite) : base(driver, python)
    {
    ...
    {
        _casinoSite.LogInfo?.Invoke("Clicking Rebet button...");
        ...
        if (!isConfirmed)        
        {
            throw new Exception("VERIFICATION FAILED: The game engine failed to verify the 'Rebet button click'.");
        }
    }
}
